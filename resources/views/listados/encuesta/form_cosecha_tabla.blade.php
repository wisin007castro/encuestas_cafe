...
<div class="col-md-8">
    ...
    <div class="box-header with-border bg-primary">
		...
		<a href="{{route('form_cosecha_agregar')}}" class="btn btn-block btn-success btn-sm">
...
<div class="box-body box-white">
    <div class="table-responsive" >
		...
		@php
			$id_cosecha_encode = base64_encode($dato->id_cosecha)
		@endphp
		 <tr role="row" class="odd">
			<td>{{ $key + 1 }}</td>
			<td>{{ f_formato($dato->updated_at) }}</td>
			<td>{{ f_formato($dato->fecha) }}</td>
			<td>{{ $dato->manual == 1 ? 'Manual' : 'Mecánica'}}</td>
			<td>
				<a href="{{route('form_cosecha_editar', ['id_cosecha' => $id_cosecha_encode])}}" class="btn-accion-tabla">
