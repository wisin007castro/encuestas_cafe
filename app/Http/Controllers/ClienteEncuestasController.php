...
class ClienteEncuestasController extends Controller
{
    ...
    public function cliente_informacion_basica_guardar(){
        ...
        $datas = \DB::table('enc_preparaciones')->get();
        if (count($datas) > 0) {
            foreach ($datas as $key => $value) {
                ...
                $this->encuestas->guardar_data($e, $url);
            }
    ...
    public function cliente_densidad_guardar(){
        ...
        $datas = \DB::table('enc_densidad')->get();
        if (count($datas) > 0) {
            foreach ($datas as $key => $value) {
                $e = array();
                    $e['object_id'] = $value->object_id;
    ...
    public function cliente_agroforestales_guardar(){
        ...
        $datas = \DB::table('enc_sist_agroforestales')->get();
        if (count($datas) > 0) {
            foreach ($datas as $key => $value) {
                $e = array();
                $e['object_id'] = $value->object_id;
    ...
    public function cliente_control_malezas_guardar(){
        ...
        $datas = \DB::table('enc_controles_maleza')->get();
        if (count($datas) > 0) {
            foreach ($datas as $key => $value) {
                $e = array();
                $e['object_id'] = $value->object_id;
